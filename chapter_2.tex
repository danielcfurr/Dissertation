...
%The insample deviance 
%$\mathrm{dev}(y^{(\mathrm{t})}, \hat \omega_m | y^{(\mathrm{t})})$
%will tend to be lower than the test deviance
%$\mathrm{dev}(y^{(\mathrm{t})}, \hat \omega_{m^*} | y^{(\mathrm{n})})$.
%Let
%\begin{equation}
%	d_{\mathrm{HV}} = \mathrm{E}_{y^{(\mathrm{t})}} \mathrm{E}_{y^{(\mathrm{n})}} 
%		[
%			\mathrm{dev}(y^{(\mathrm{t})}, \hat \omega_m | y^{(\mathrm{n})}) -
%			\mathrm{dev}(y^{(\mathrm{t})}, \hat \omega_m | y^{(\mathrm{t})})
%	  ]
%\end{equation}
%be the expected difference. Given that in this chapter the test data always contains a new set of items, 
%$y^{(\mathrm{n})}$
...
%Figure~\ref{fig:2-recovery} provides summaries of the differences between estimated and generating parameters for Model~2 across simulation conditions, for example, $\hat \beta_1 - \beta_1$. In the interest of studying parameter recovery, focus is placed on the correct model, Model~2. Systematic deviations from a difference of zero imply bias. In the top panel of Figure~\ref{fig:2-recovery}, it may be seen that for $\tau = .2$ the parameter estimates are unbiased with the LLTM, though all estimates show a small amount of bias as $\tau$ increases. For $\tau = 1$, all parameter estimates tend to be closer to zero (consider that $\beta_1$ and $\beta_5$ are negative, while all other parameters are positive), including $\hat \sigma$. The LLTM-E2S avoids this bias for the $\hat \beta$'s, though $\hat \tau$ does exhibit a downward bias that becomes more pronounced with higher values of $\tau$. (The LLTM does not include estimation of $\tau$, and LLTM-E2S does include estimation of $\sigma$ in the final results.)
%
%The bottom panel of Figure~\ref{fig:2-recovery} presents the same information for varying numbers of items. The number of items does not seem to effect the slight bias resulting from the LLTM. For the two-stage method, larger numbers of items appears to reduce the downward bias of $\tau$. \emph{((give some bias estimates and p-values))}

%\begin{figure}
%	\centering
%	\includegraphics{figs/2-recovery}
%	\caption{Box plots of estimated minus generating values for the parameters of Model~2 across simulation conditions. The LLTM does not include estimation of $\tau$, and the LLTM-E2S does not include estimation of $\sigma$ in the second stage.}		
%	\label{fig:2-recovery}
%\end{figure}
...
%While the LLTM and LLTM-E2S yield similar point estimates, the two approaches provide very different standard error estimates. Here only the standard errors for the $\hat \beta$'s are considered. Those resulting from the LLTM are much smaller than those from the two stage estimation. The accuracy of a standard error may be studied by calculating the percentage of replications in which the parameter estimates are within one standard error of the generating values. If the estimates are unbiased and the standard errors correct, then 68\% of replications should capture the generating values of parameter in this way. Figure~\ref{fig:2-capture} presents the percentages for each parameter by simulation condition. The LLTM captures the true parameters at rates much lower 68\% in all conditions. The LLTM-E2S yields capture rates that are also too low, but they are close to the correct 68\%.
%
%\begin{figure}
%	\centering
%	\includegraphics{figs/2-capture}
%	\caption{Percentage of replications in which the estimated parameter is within $\pm 1$ standard error of the generating value for Model~2. The dashed line is at 68\%, which indicates the expected result if the parameter estimates are unbiased and the standard errors correct.}		
%	\label{fig:2-capture}
%\end{figure}
...
%\begin{figure}
%	\centering
%	\includegraphics{figs/2-hvpred-twostage}
%	\caption{Kernal density plots of the difference in holdout validation deviance using new versus the same items for the selected models 
%		($\mathrm{dev}(y^{(\mathrm{t})}, \hat \omega_{m^*} | y^{(\mathrm{n})}) -
%		\mathrm{dev}(y^{(\mathrm{t})}, \hat \omega_{m^*} | y^{(\mathrm{s})})$).}
%	\label{fig:2-hvpred-twostage}
%\end{figure}
