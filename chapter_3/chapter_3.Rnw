...
# Subset the data frame to only IC estimates
df_estimates <- subset(df, variable == "estimate")

# Obtain a second dataframe of fake points that have same range in the
# conditional and marginal case
df_min <- dcast(df_estimates, focus ~ ., min)
names(df_min)[names(df_min)=="."] <- "value"
df_max <- dcast(df_estimates, focus  ~ ., max)
names(df_max)[names(df_max)=="."] <- "value"
ranges <- df_max$value - df_min$value
adjustment <- (max(ranges) - ranges) / 2
df_min$value <- df_min$value - adjustment
df_max$value <- df_max$value + adjustment

# Make the fake data frame match the real one.
df_adjusted <- rbind(df_min, df_max)
df_adjusted$variable = "estimate"
df_adjusted$model <- df_estimates$model[1] # Arbitrary
df_adjusted$criterion <- df_estimates$criterion[1] # Arbitrary

# Plot real and fake dataframes. Fake data frame puts both axes on same width.
ggplot() +
  geom_jitter(height = 0, width = .1, alpha = .25,
              mapping = aes(y = value, x = as.factor(model)),
              data = df_estimates) +
  geom_point(mapping = aes(y = value, x = as.factor(model)),
             data = df_adjusted, alpha = 0) +
  facet_grid(focus ~ Criterion, scale = "free_y") +
...
{Information criteria values ($-2 \widehat \mathrm{elpd}$) for the five latent regression Rasch models. Points represent the results of the $\Sexpr{n_repeats}$ independent MCMC simulations per model. A small amount of horizontal jitter is added to the points. The $y$-axes vary by focus but have the same width.}
