...
\begin{equation}
  ...
  p(\omega, \psi)
,\end{equation}
where $y_{ij} \in y$ is the response for observation $i$ ($i = 1 \ldots I$) in cluster $j$ ($j = 1 \dots J$), $\omega$ is a vector of parameters common across clusters, $\zeta_j$ is a cluster-specific parameter, and $\psi$ is a hyperparameter for the prior distribution of $\zeta_j$. Further, let $\zeta$ represent the vector containing all $\zeta_j$. The likelihood $p(y_{ij} | \omega, \zeta_j)$ is on $\zeta_j$ and therefore does not directly involve $\psi$. The prior distributions included the hierarchical prior for $\zeta$ and the joint prior for $\omega$ and $\psi$, which may be rewritten as $p(\omega) p(\psi)$ if independent priors are assigned.
...
Information criteria estimate the $\mathrm{elpd}_c$ using a single dataset. DIC requires both the mean of the log likelihood taken over the posterior and the log likelihood evaluated at the posterior mean. The mean of the log likelihood is
\begin{equation}
...
The log likelihood evaluated at the posterior mean is estimated as
\begin{equation} \label{eq:3-best-lpdc}
  \widehat{\mathrm{lpd}}_{\mathrm{DIC},c}^* =
    \sum_{j=1}^J \sum_{i=1}^I
    \log p(y_{ij} | \bar \omega, \bar \zeta_j)
,\end{equation}
where $\bar \omega$ and $\bar \zeta_j$ refer to the estimated posterior means.
...
The purpose of DIC is to optimize predictions rather than identify a true model \parencite[][p. 4]{spiegelhalter2014deviance}, and the same is true for other information criteria motivated by cross-validation. % , and in this way is analogous to AIC rather than BIC.


\subsubsection{Conditional widely applicable information criteria}

WAIC is a form of Bayesian information criterion that does not rely on point estimates for the parameters and requires only the log pointwise predictive density, which is:
\begin{equation}
  \mathrm{lpd}_c =
  \sum_{j=1}^J \sum_{i=1}^I
  \log \iiint
  ...
  ~\mathrm{d}\omega \mathrm{d}\psi \mathrm{d}\zeta_j
.\end{equation}
The $\mathrm{lpd}_c$ is the full data log-likelihood integrated over the joint posterior for all parameters, and it differs from $\mathrm{lpd}_{\mathrm{DIC},c}$ in that it involves the log of an integrated probability rather than an integrated log probability.
...
\begin{equation}
  ...
  \right )
.\end{equation}
%Using $\widehat \mathrm{lpd}_c$ to evaluate predictive performance will overstate the accuracy because the same data are used to fit the model and evaluate the predictions.
The estimated effective number of parameters associated with WAIC is
...
,\end{equation}
where $y_{j}$ is the vector of responses for cluster $j$, and $p(\tilde \zeta_{j'} | \psi)$ is the prior distribution of $\zeta_j$ given by $\psi$. This prior is not directly influence by $y_{j}$ in contrast to $p(\zeta_j | y_j, \psi)$.
...
is similar to
$p(y_{j} | \omega, \psi)$
(in Equation~\ref{eq:3-marginal-likelihood}) but for unobserved $\tilde y_{j'}$ and $\tilde \zeta_{j'}$. The density $p(\tilde y_{j'} | y)$ may be referred to as a mixed predictive distribution \parencite{Gelman1996, marshall2007identifying}; it involves the posterior for $\omega$ and $\psi$ but the prior for $\zeta_j$.
...
Marginal information criteria are calculated in much the same way as their conditional counterparts by substituting $p(y_{j} | \omega^s, \psi^s)$ for $p(y_{ij} | \omega^s, \zeta_j^s)$. For marginal DIC, the mean of the log likelihood is
\begin{equation}
...
\begin{equation}
  ...
  \right ]
.\end{equation}
The marginal log-likelihood evaluated at the posterior means of the parameters is also required:
...
\section{Simulation study of adaptive quadrature and circular block boot}
...
df_both <- cbind(df_mvn, df_q[,-1:-2])
xtab_both <- xtable(df_both, display = c("d","d","s","f","f","f","f","d","d"))
...
@
\caption{Conditional and marginal DIC, WAIC, and PSIS-LOO for the simulated datasets using the multivariate normal density. In the conditional focus, there are a total of
\Sexpr{p_c}
...
@
\end{center}
\caption{Differences in marginal information criteria ($-2 \times \widehat{\mathrm{elpd}}_m$) between calculations using adaptive quadrature (approximate method) and the multivariate normal density function (exact method). The $y$-axis uses a log scale. The dashed line is drawn at .01.}
\label{fig:3-compare}
\end{figure}
...
print(xtab_bf, include.rownames = FALSE, floating = FALSE,
      ...
      sanitize.colnames.function = function(x) x)
@
\caption{Means and standard deviations for the ``brute force'' WAIC results for the simulation. Results are from \Sexpr{length(sim_bs_l)} independent sets of MCMC chains using the same simulated dataset with cluster size $I=\Sexpr{sim_I[1]}$.}
\label{tab:3-sim-bf}
\end{table}

Next the circular block bootstrap is applied once each to the \Sexpr{length(sim_bs_l)} independent sets of MCMC chains. Block sizes ranging from \Sexpr{min(sim_bs_l)} to \Sexpr{max(sim_bs_l)} were used, and the results are presented in Figure~\ref{fig:3-bootstrap}. The bootstrap substantially underestimates the Monte Carlo error for both conditional and marginal WAIC. While Monte Carlo error estimates for $\mathrm{lpd}_\mathrm{WAIC}$ and $p_\mathrm{WAIC}$ are both too small using the bootstrap, the discrepancy for $p_\mathrm{WAIC}$ is much worse. Further, block size does not appear to have an effect on estimated Monte Carlo error, possibly because autocorrelations with Stan are typically low. This bootstrap scheme may also be applied to DIC and PSIS-LOO, but there is no reason to expect better performance.
...
Marginal DIC, WAIC, and PSIS-LOO were computed using adaptive quadrature to integrate out $\zeta$. Arbitrarily focusing on Model~4, the process described in the previous section was used to determine that \Sexpr{n_agq} nodes were sufficient to obtain an accurate approximation.
...
Figure~\ref{fig:3-effn} provides the estimated effective number of parameters ($\hat p$) for each model and focus, as well as the count of parameters associated with either focus.
...
@
\end{center}
\caption{Information criteria values ($-2 \widehat \mathrm{elpd}$) for the five latent regression Rasch models. Points represent the results of the $\Sexpr{n_repeats}$ independent MCMC simulations per model. A small amount of horizontal jitter is added to the points. The $y$-axes vary by focus.}
\label{fig:3-example-ic}
\end{figure}

As discussed in their respective sections, WAIC and PSIS-LOO are associated with diagnostics to support the reliability of their estimates. In either case, the diagnostics are specified at the observation level. In the conditional focus, observations are defined at the unit level, whereas they are defined at the cluster level for the marginal focus. For WAIC any given observation should contribute less than .4 to $\hat p$, and for PSIS-LOO an observation should not have a Pareto shape parameter greater than one. Table~\ref{tab:3-diagnostics} provides the counts of problematic observations by model, averaging over the \Sexpr{n_repeats} repreated MCMC simulations. Conditional WAIC has a small number problematic observations with each model, while marginal WAIC does not exhibit any such issues. No problematic observations are found for either form of PSIS-LOO.
...
The preceding analyses demonstrated the existence of non-ignorable Monte Carlo error in the forms of information criteria considered. Caution is therefore advised when using DIC, WAIC, or PSIS-LOO for selecting a preferred model. Disappointingly, the circular block bootstrap did not provide reasonable estimates for Monte Carlo error of information criteria estimates. The approach taken in the applied example of simply rerunning the MCMC simulation many times is cumbersome and merely suggestive of the amount of Monte Carlo error, but it could provide accurate results if the computing power is available to conduct a much larger number of replications.

The choice of conditional or marginal focus should depend on the prediction inference to be made, but the marginal approach was found to have some advantages over the conditional approach. Marginal information criteria were found to have less Monte Carlo error and to be more robust in terms of their associated diagnostics. The conditional information criteria are more easily obtained, however, as they depend on quantities easily generated from MCMC software. In addition, BUGS and JAGS provide conditional DIC by default, perhaps accounting for the popularity of DIC.

Marginal information criteria are easily obtained for linear models when the cluster-specific parameters to be integrated out are assigned a normal prior. For non-linear models, adaptive gaussian quadrature was shown to be a viable means of obtaining the necessary marginal likelihoods. The methods described in this chapter may be extended to models with cluster-specific vectors of parameters having a multivariate normal prior.
