...
.\end{equation}
This marginal WAIC corresponds partially to the iWAIC of \textcite{li2015approximating}, though they did not consider the case of clustered data.
...
print(xtab_bf, include.rownames = FALSE, floating = FALSE,
      ...
      sanitize.colnames.function = function(x) x)
@
\caption{Means and standard deviations for the ``brute force'' results for WAIC for the simulation. Results are from \Sexpr{length(sim_bs_l)} independent sets of MCMC chains using the same simulated dataset ($I=\Sexpr{sim_I[1]}$).}
\label{tab:3-sim-bf}
\end{table}
...
Table~\ref{tab:3-effn} provides the estimated effective number of parameters for each model and focus, as well as the count of parameters associated with either focus.
The effective numbers of parameters are substantially less than the counts of parameters for conditional information criteria, owing mainly to the fact that each $zeta_j$, with its hierarchical prior, contributes less than one to the effective number of parameters.
On the other hand, the values for marginal information criteria are close to the counts of model parameters.
%Estimated effective numbers of parameters for conditional information criteria are much greater because each $\zeta_j$ makes a contribution instead of the single contribution of $\sigma$. Depending on the informativeness of the prior distribution, the vector $\zeta$ will contribute between 1 (if the prior is absolutely informative) and $J$ (if totally uninformative) effective parameters.

\begin{table}[htb]
\centering
<<parameters, results = tex>>=
# keep <- c(keep, paste0(keep, "_mcerror"))
df <- subset(df_models, variable %in% c("p_dic", "p_waic", "p_loo"))
df$Criterion <- toupper(gsub("(p\\_)", "", df$variable))
df$Criterion <- gsub("(LOO|LOOIC)", "PSIS-LOO", df$Criterion)
df$variable <- NULL
df <- aggregate(value ~ focus + model + Criterion, df, mean)
...
df_par_counts <- data.frame(focus = rep(c("Marginal", "Conditional"), each = 5),
                            ...
                            Criterion = rep("Count", 10),
                            value = c(marginal_pars[1:5], conditional_pars[1:5]))
df <- rbind(df, df_par_counts)
df$Criterion <- factor(df$Criterion, c("DIC", "WAIC", "PSIS-LOO", "Count"))

df_tab <- dcast(df, focus + Criterion ~ model)
names(df_tab) <- c("Focus", "Criterion", paste("Model", 1:5))
...
print(xtable(df_tab),
      ...
      floating = FALSE)
@
\caption{Estimated effective number of parameters $\hat p$) for the five models using various information criteria. The counts of parameters associated with each focus are also provided.}
\label{tab:3-effn}
\end{table}

Figure~\ref{fig:3-example-ic} provides the estimates of conditional and marginal DIC, WAIC, and PSIS-LOO. The different conditional information criteria differ from each other for any given model, though the three of them suggest the same ranking of models. Marginal WAIC provides nearly the same results as marginal PSIS-LOO, while values for marginal DIC are consistently less. The model rankings differ somewhat between the conditional and marginal focus, though small difference between competing models should not be over-interpreted due to the unknown degree of Monte Carlo error.

\begin{figure}
\begin{center}
<<mcerror_plot, fig = TRUE>>=
df <- subset(df_models, variable %in% c("dic", "waic", "looic"))
df$Criterion <- toupper(df$variable)
df$Criterion <- gsub("(LOO|LOOIC)", "PSIS-LOO", df$Criterion)
df$Criterion <- factor(df$Criterion, c("DIC", "WAIC", "PSIS-LOO"))
ggplot(df) +
...
@
\end{center}
\caption{Information criteria values for the five latent regression Rasch models by focus.}
\label{fig:3-example-ic}
\end{figure}

<<mcerror_numbers>>=
# For reporting in text, get ranges of IC values
# ic_range <- aggregate(value ~ focus + Criterion, data = df,
#                       FUN = function(x) max(x) - min(x))
# ic_range_cond <- ic_range[ic_range$focus == "Conditional", "value"]
# ic_range_marg <- ic_range[ic_range$focus == "Marginal", "value"]
