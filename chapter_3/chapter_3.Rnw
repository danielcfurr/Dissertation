...
For the conditional approach, we evaluate predictive quantities conditional on $\zeta_j$, while we integrate out $\zeta_j$ for the marginal approach.
...
,\end{equation}
and the $c$ subscript in $\mathrm{lpd}_c$ denotes that it is based on the conditional likelihood $p(y_{ij} | \omega, \zeta_j)$. The $\mathrm{lpd}_c$ is the (log of) the full data likelihood integrated over the joint posterior for all parameters.
...
\begin{equation}
  ...
  \sum_{j=1}^J \sum_{i=1}^I
    \log \frac{1}{S} \sum_{s=1}^S p(y_{ij} | \omega^s, \zeta_j^s)
,\end{equation}
where $\omega^s$ and $\zeta_j^s$ represent parameter values at posterior draw $s$, $s = 1 \cdots S$. \textcite{vehtari2016practical} do not include the $c$ subscripts on $\mathrm{lpd}_c$ or the other quantities that follow in this section.
...
Information criteria and cross-validation are means of estimating the expected log pointwise predictive density. Using $\widehat \mathrm{lpd}_c$ to evaluate predictive performance will overstate the accuracy because the same data are used to fit the model and evaluate the predictions. Information criteria typically apply a penalty to $\widehat \mathrm{lpd}_c$ related to the ``effective number of parameters.''

Information criteria estimate the $\mathrm{elpd}_c$ using a single dataset, and these come in many forms.
The deviance information criteria \parencite[DIC;][]{spiegelhalter2002bayesian} requires evaluating the log pointwise predictive density at the posterior means of the parameters, which is
\begin{equation} \label{eq:2-best-lpdc}
  \widehat{\mathrm{lpd}}_c^* =
    \sum_{j=1}^J \sum_{i=1}^I
    \log p(y_{ij} | \bar \omega, \bar \zeta_j)
,\end{equation}
where $\bar \omega$ and $\bar \zeta_j$ refer to the posterior means for their respective parameters. This reliance on point estimates is a source of criticism \emph{((citations))}.
...
\begin{equation}
  ...
  \widehat{\mathrm{lpd}}_c^* - \hat p_{c,\mathrm{DIC}}
.\end{equation}
...
Widely applicable information criteria \parencite[WAIC;][]{watanabe2010asymptotic} is a form of Bayesian information criterion that does not rely on point estimates for the parameters. The estimated effective number of parameters associated with WAIC is
\begin{equation}
  \hat p_{c,\mathrm{WAIC}} =
  \sum_{j=1}^J \sum_{i=1}^I V_{s=1}^S \log p(y_{ij} | \omega^s, \zeta_j^s)
\end{equation}
\parencite{vehtari2016practical} where $V_{s=1}^S$ represents the sample variance across the $S$ iterations. With this estimate of the effective number of parameters, the expected log pointwise predictive density for WAIC is
\begin{equation}
  \widehat \mathrm{elpd}_{c,\mathrm{WAIC}} =
  \widehat{\mathrm{lpd}}_c - \hat p_{c,\mathrm{WAIC}}
.\end{equation}
An alternative formula for the WAIC estimate of the effective number of parameters based on the mean of $p(y_{ij} | \omega^s, \zeta_j^s)$ is available \parencite[for example,][p. 1002]{gelman2014understanding}, but it is not used here.
...
\begin{equation}
  ...
  ~\mathrm{d} \omega \mathrm{d} \psi \mathrm{d} \zeta_j
,\end{equation}
which bears resemblance to the posterior predictive distribution for a new response. An estimate of $\mathrm{elpd}_{c,\mathrm{LOO}}$ may be obtained from the posterior draws as
\begin{equation}
  \widehat{\mathrm{elpd}}_{c, \mathrm{LOO}} =
...
The calculation of the raw importance weights was developed by \textcite{gelfand1992model}, the truncation of the weights was developed by \textcite{ionides2008truncated}, and the Pareto smoothing was developed by \textcite{vehtari2016pareto}. Because $\widehat{\mathrm{elpd}}_{c,\mathrm{LOO}}$ is calculated from a single fit of a model on one dataset, it may be considered a form of information criteria. Lastly, an estimate of the effective number of parameters from PSIS-LOO may be obtained after the fact by subtracting $\widehat{\mathrm{lpd}}_c$ from $\widehat{\mathrm{elpd}}_{c, \mathrm{LOO}}$.
...
\begin{equation} \label{eq:3-marginal-likelihood}
  ...
  \int
    p(\tilde \zeta_j | \psi)
    \prod_{i=1}^I p(y_{ij} | \omega, \tilde \zeta_j)
  ~\mathrm{d} \tilde \zeta_j
,\end{equation}
where $y_{j}$ is the vector of responses for cluster $j$, and $\tilde \zeta_j$ is a draw from the prior distribution indicated by $\psi$. The posterior for $\tilde \zeta_j$, $p(\tilde \zeta_j | \psi)$ is not directly influence by $y_{j}$, in contrast to $p(\zeta_j | y_j, \psi)$.
...
$p(y_{j} | \omega, \psi)$
but for a given posterior sample $s$. Here the ``points'' in ``pointwise'' are clusters rather than individual observations, which is another difference between the marginal and conditional approaches.
The $m$ subscript indicates that the $\mathrm{lpd}_m$ and $\widehat{\mathrm{lpd}}_m$ results from the marginal likelihood.
...
,\end{equation}
where
$p(\tilde y_{j'} | \omega, \tilde \zeta)$
is similar to
$p(y_{j} | \omega, \zeta_j)$
but for unobserved $\tilde y_{j'}$ and $\tilde \zeta$. The density $p(\tilde y_{j'} | y)$ may be referred to as a mixed predictive distribution \parencite{Gelman1996, marshall2007identifying} as it relies on $\tilde \zeta$, which is influenced by $\psi$ but not $\tilde y_{j'}$. The expected log pointwise predictive density for a new dataset, containing a new sample of clusters, is
\begin{equation}
  \mathrm{elpd}_m =
  \sum_{j=1}^J \sum_{i=1}^I
  \int
...
Marginal WAIC and PSIS-LOO are calculated in much the same way as their conditional counterparts by simply substituting $p(y_{j} | \omega^s, \psi^s)$ for $p(y_{ij} | \omega^s, \zeta_j^s)$. All that is required for WAIC and PSIS-LOO is conditionally independent likelihoods for each observation at each posterior draw, and ``observation'' may be defined at the unit or cluster level. DIC is much the same, except that it also requires evaluating the likelihood at the posterior means of the parameters. In this way, what makes DIC, WAIC, and PSIS-LOO marginal instead of conditional is merely the form of the likelihood fed into the calculations and the redefinition of "points." For completeness, equations related to marginal information criteria will be shown nonetheless. The marginal likelihood evaluated at the posterior means of the parameters is
\begin{equation} \label{eq:2-best-lpdm}
  \widehat{\mathrm{lpd}}_m^* =
    \sum_{j=1}^J
    \log p(y_{j} | \bar \omega, \bar \psi)
,\end{equation}
and so the estimated effective number of parameters for DIC is
\begin{equation}
  \hat p_{m,\mathrm{DIC}} =
  2 \widehat{\mathrm{lpd}}_m^* - 2 \widehat{\mathrm{lpd}_m}
,\end{equation}
which may be subtracted from $\widehat{\mathrm{lpd}}_m^*$ to obtain $\widehat \mathrm{elpd}_{m,\mathrm{DIC}}$.
% Then the expected log pointwise predictive density for DIC is
% \begin{equation}
%   \mathrm{elpd}_{\mathrm{DIC}, m} =
%   \widehat{\mathrm{lpd}}_m^* - \hat p_{m,\mathrm{DIC}}
% .\end{equation}
...
,\end{equation}
which may be subtracted from $\widehat{\mathrm{lpd}}_m$ to obtain $\widehat \mathrm{elpd}_{m,\mathrm{WAIC}}$. The marginal Bayesian leave-one-out expected log pointwise predictive density is
\begin{equation}
  \mathrm{elpd}_{m,\mathrm{LOO}} = \sum_{j=1}^J \log p(y_{j} | y_{-j})
\end{equation}
...
\begin{equation}
  ...
  ~\mathrm{d} \omega \mathrm{d} \psi
,\end{equation}
which bears resemblence to the mixed predictive distribution for $\tilde y_{j'}$. An estimate of $\mathrm{elpd}_{m,\mathrm{LOO}}$ may be obtained from the posterior draws as
\begin{equation}
...
@
\end{center}
\caption{Differences in marginal information criteria ($-2 \times \widehat{\mathrm{elpd}}_m$) between calculations using adaptive quadrature (approximate method) and the multivariate normal density function (exact method). The horizontal line is drawn at .01.}
\label{fig:3-compare}
\end{figure}
...
@
\end{center}
\caption{Differences in marginal information criteria ($-2 \times \widehat{\mathrm{elpd}}_m$) between using $M$ adaptive quadrature nodes and $\approx \frac{2}{3}M$ nodes. The horizontal line is drawn at .01.}
...
kept posterior draws. The $\hat R$ convergence statistic was \Sexpr{f(max_rhat,2)} or less for all parameters and the log posterior across models. DIC, WAIC, and LOO-IC were computed on the conditional and marginal likelihoods.

Adaptive quadrature was used to approximate the marginal likelihoods. Arbitrarily focusing on Model~4, the full data marginal log-likelihood, $\mathrm{lpd}_m$, was evaluated using \Sexpr{and(n_agq_try)} nodes to determine a sufficient number of adaptive quadrature nodes. Using the rule to select the smaller number of nodes when the difference in each of DIC, WAIC, and LOO-IC is less than .01, \Sexpr{n_agq} nodes were found to be sufficient. All evaluations of the marginal likelihood that follow use this number of nodes.

From a freqentist perspective on generalized linear mixed models, there are $I-1$ unconstrained parameters in $\delta$, one for $\sigma$, and one parameter per element in $\lambda$. Meanwhile, each $\zeta_j$ would marginalized out in maximum marginal likelihood estimation, and so would not be counted. It follows that in this perspective
...
df$ic_type <- gsub("(LOO|LOOIC)", "LOO-IC", df$ic_type)
...
df$ic_type <- factor(df$ic_type, c("DIC", "WAIC", "LOO-IC"))
...
The time series boostrap is used to obtain Monte Carlo error estimates for DIC and WAIC. The block size $S^\frac{1}{3} \approx \Sexpr{round(n_posterior^(1/3))}$ is chosen as the default and is compared against $\Sexpr{round(n_posterior^(1/3))} \pm 5$ to ascertain how sensitive the estimated Monte Carlo error is to choice of block size. Further, to obtain some indication of the variability of the bootstrap results, the procedure is repeated \Sexpr{n_block_size_reps} times. Figure~\ref{fig:blocksize} presents the resulting estimates of Monte Carlo error for conditional and marginal DIC and WAIC, obtained for Model~4. The bootstrap is not applied to to LOO-IC, as LOO-IC is more computationally intensive than DIC or WAIC and so may require an unreasonable amount of time to bootstrap. For all information criteria considered, the estimated Monte Carlo errors do not appear to depend much on block size. Further, it may be seen that the estimated Monte Carlo errors are substantially smaller for the marginal forms of information criteria.
...
@
\end{center}
\caption{Information criteria values for the five models with $\pm 2$ Monte Carlo errors indicated by vertical lines (not calculated for LOO-IC).}
\label{fig:mcerror}
\end{figure}
