...
expand_bias <- range(subset(df_bias, model == 2, c("lower", "upper")))
...
expand_qq <- range(subset(df_recov, par == "beta6", "z"))
...
Two ways of assessing the accuracy of item predictions may be considered. The root mean squared error of prediction for model $m$ is
\begin{equation}
	\mathrm{RMSE}_m = 
	\sqrt{\frac{1}{I} \sum_{i=1}^I 
	  \left [ 
	    x_i'\hat\beta_m(y^\mathrm{t}) - \delta_i(y^\mathrm{e}) 
	  \right ]^2}
,\end{equation}
where $\hat\beta_m(y^\mathrm{t})$ is a vector of coefficients for model $m$ estimated on the training subset $y^\mathrm{t}$ and $\delta_i(y^\mathrm{e})$ are true item difficulties (fixed plus residual parts) associated with the evaluation data $y^\mathrm{e}$. The $\mathrm{RMSE}_m$ is based on the difference between the predicted item difficulty (without residual) and the true item difficulty (with residual) in a different subset. The ``fixed'' root mean squared error of prediction for model $m$ is
\begin{equation}
	\mathrm{FRMSE}_m = 
	\sqrt{\frac{1}{I} \sum_{i=1}^I 
	  \left [ 
	    x_i'\hat\beta_m(y^\mathrm{t}) - x_i'\beta) 
	  \right ]^2}
,\end{equation}
where $\beta$ are the true coefficients. The $\mathrm{FRMSE}_m$ is based on the difference between the predicted item difficulty (without residual) and fixed part of the true item difficulty (without residual). In both equations, $x_i$ are predictors from the evaluation subset, though in this simulation $x_i$ are the same across subsets. Results for $\mathrm{RMSE}_m$ are highly dependent on the residual standard deviation $\sigma$, so I report $\mathrm{FRMSE}_m$. Further, let $\mathrm{FRMSE}_{m^*}$ be the fixed root mean squared error of prediction for the model chosen as best by a given selection strategy.

Figure~\ref{fig:2-frmse} presents the fixed root mean squared error of prediction for the model chosen by the several selection strategies. Predictably, the high information simulation conditions yield much smaller prediction errors in comparison to the low information conditions. The LLTM-E2S tends to make better predictions than the LLTM, but this difference is generally quite small. Likewise, holdout validation with new items better than the other selection approaches but only by a very small degree.

<<frmse_setup>>=
df_frmse_expand <- range(df_frmse$value)
frmse_1 <- ggplot(subset(df_frmse, nitems == 32)) +
  aes(y=value, x = tau, pch = Method, lty = Method) +
  geom_point() + geom_line() +
  facet_wrap(~Selector, nrow = 1) +
  xlab(expression(paste("Residual item SD (", tau, ")"))) + 
  ylab(expression(paste(FRMSE[italic(m)^symbol("\52")]))) +
  scale_x_continuous(breaks = unique(df_frmse$tau)) +
  expand_limits(y = df_frmse_expand) +
  my_theme + 
  theme(panel.grid.minor.x = element_blank())
frmse_2 <- ggplot(subset(df_frmse, tau == .5)) +
  aes(y = value, x = nitems, pch = Method, lty = Method) +
  geom_point() + geom_line() +
  facet_wrap(~Selector, nrow = 1) +
  xlab(expression(paste("Number of items (", italic(I), ")"))) +
  ylab(expression(paste(FRMSE[italic(m)^symbol("\52")]))) +
  scale_x_continuous(breaks = unique(df_frmse$nitems)) +
  expand_limits(y = df_frmse_expand) +
  my_theme + 
  theme(panel.grid.minor.x = element_blank())
# grid_arrange_shared_legend(frmse_1, frmse_2, ncol = 1, nrow = 2, 
#                            position = "bottom")
grid.arrange(frmse_1, frmse_2, ncol = 1)
@
\begin{figure}[tb]
\begin{center}
<<frmse_plot, fig = TRUE>>=
...
@
\end{center}
\caption{The mean fixed root mean squared error of prediction for the selected model $\mathrm{FRMSE}_m$ across simulation replications. The top panel column shows simulation replications in which the residual item standard deviation ($\tau$) varies while the number of items is held at 32, and the bottom panel shows replications in which the number of items varies while $\tau$ is held at .5.}
\label{fig:2-frmse}
\end{figure}

Figure~\ref{fig:2-frmse-m}

\begin{figure}[tb]
\begin{center}
<<frmse_m_plot, fig = TRUE>>=
df_frmse_m_expand <- range(df_frmse_m$value)
frmse_m_1 <- ggplot(subset(df_frmse_m, nitems == 32)) +
  aes(y=value, x = tau, color = Model, pch = Model) +
  geom_point() + geom_line() +
  stat_summary_bin(geom="line", fun.y = mean)  +
  facet_wrap(~Method, scales = "free_y") +
  xlab(expression(paste("Residual item SD (", tau, ")"))) + 
  ylab(expression(FRMSE[m])) +
  scale_x_continuous(breaks = unique(df_frmse_m$tau)) +
  expand_limits(y = df_frmse_m_expand) +
  my_theme + 
  theme(panel.grid.minor.x = element_blank())
frmse_m_2 <- ggplot(subset(df_frmse_m, tau == .5)) +
  aes(y=value, x = nitems, color = Model, pch = Model) +
  geom_point() + geom_line() +
  facet_wrap(~Method, scales = "free_y")+
  xlab(expression(paste("Number of items (", italic(I), ")"))) +
  ylab(expression(FRMSE[m])) +
  scale_x_continuous(breaks = unique(df_frmse_m$nitems)) +
  expand_limits(y = df_frmse_m_expand) +
  my_theme + 
  theme(panel.grid.minor.x = element_blank())
# grid_arrange_shared_legend(frmse_m_1, frmse_m_2, ncol = 1, nrow = 2, 
#                            position = "bottom")
grid.arrange(frmse_m_1, frmse_m_2, ncol = 1)
@
\end{center}
\caption{The top panel column shows simulation replications in which the residual item standard deviation ($\tau$) varies while the number of items is held at 32, and the bottom panel shows replications in which the number of items varies while $\tau$ is held at .5.}
\label{fig:2-frmse-m}
\end{figure}
